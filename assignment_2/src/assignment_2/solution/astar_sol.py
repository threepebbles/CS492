...
def astar_planning(start, goal, actions, resolution, grid_limits,
    ...
    while True:
        ...
        for i, action in enumerate(actions): 
            ...
            if is_valid(next_pos, grid_limits, obstacle_tree, robot_size) is False:
                continue
            
            dist1 = np.linalg.norm(next_pos - cur_node.pos)
            dist2 = np.linalg.norm(next_pos - goal)
            
            node = Node(next_pos,
                        get_grid_index(next_pos, resolution, grid_limits, grid_dim),
                        cur_node.cost + dist1,
                        dist2,
                        cur_idx)
            if node.idx in closedset: continue
            
            # Otherwise if it is already in the open set
            if node.idx in openset:
                if openset[node.idx].cost > node.cost:
