...
class ArmClient(object):
    ...
    def move_joint(self, angles, duration=3.):
        ...
        # ------------------------------------------------------
        for p, v ,t in zip(pos, vel, time):
            g.trajectory.points.append(
                JointTrajectoryPoint(positions=p, velocities=v,
    ...
    def move_position(self, pose, duration=5.):
        ...
        for i, t in enumerate(time):
            ...
            # ------------------------------------------------------
            # get delta position
            dx = np.array([poses[i].position.x-poses[i-1].position.x,
    ...
    def move_pose(self, pose, duration=5.):
        ...
        for i, p in enumerate(progress):
            ...
            pose.position.z = (1.-p)*start_pose.position.z + p*goal_pose.position.z
            # orientation (SLERP)
            pose.orientation = quaternion.slerp(start_pose.orientation,
        ...
        for i, t in enumerate(time):
            ...
            # ------------------------------------------------------
            # get delta position
            pos_delta = frame.p-prev_frame.p
            # get delta orientation
            rot_delta = frame.M*prev_frame.M.Inverse()
            rpy_delta = rot_delta.GetRPY()
            dx = np.array([pos_delta[0], pos_delta[1], pos_delta[2],
