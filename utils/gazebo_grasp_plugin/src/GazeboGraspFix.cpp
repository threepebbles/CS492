...
void GazeboGraspFix::InitValues()
{
  ...
  this->gripperCancelPub = this->rosnode->advertise<actionlib_msgs::GoalID>("/gripper_controller/gripper_cmd/cancel", 10);
...
void GazeboGraspFix::OnUpdate()
{
       ...
       ocIt != objectContactInfo.end(); ++ocIt)
  {
    ...
    // CUSTOM: send a topic to stop closing
    actionlib_msgs::GoalID goal_id_msg;    
    goal_id_msg.stamp = ros::Time::now();
    goal_id_msg.id    = std::string("gripper");
    this->gripperCancelPub.publish(goal_id_msg);
    gzmsg << "GazeboGraspFix: Cancelling the grasping motion" << std::endl;
    ...
    // gzmsg << "Attach the object" << std::endl;
  }  // for all objects
