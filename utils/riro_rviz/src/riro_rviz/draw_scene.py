...
class SceneDraw:
    def __init__(self, topic='sim/viz/bodies', frame='/world', latch=False):
        self.pub = rospy.Publisher(topic, Marker, queue_size=3, latch=latch)
        self.frame = frame
        self.Marker = Marker
    ...
    def pub_points(self, lPos, color, num):
        marker = Marker()
        marker.header.frame_id = self.frame
        marker.header.stamp = rospy.Time.now()
    ...
    ## def pub_pose_trajectory(self, poses, color, num):
    ##     marker = Marker()
    ##     marker.header.frame_id = self.frame
    ##     marker.header.stamp = rospy.Time.now()
